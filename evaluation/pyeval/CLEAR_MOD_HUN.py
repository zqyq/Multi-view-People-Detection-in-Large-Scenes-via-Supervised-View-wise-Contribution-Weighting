...
def CLEAR_MOD_HUN(gt, det, dist_thres=80):
    ...
    F = int(max(gt[:, 0])) + 1  # 最大帧数
    N = int(max(det[:, 1])) + 1  # 最大帧数
    Fgt = int(max(gt[:, 0])) + 1
    Ngt = int(max(gt[:, 1])) + 1  # 最多真实对象数
    ...
    for t in range(1, F + 1):
        ...
        if GTsInFrame is not None and DetsInFrame is not None:  # 求检测对象与真实对象两两之间的距离
            ...
            if not tmpai.all() == 1e6:
                ...
                u, v = HUN_res[HUN_res[:, 1].argsort()].T  # u为行，v为列，增序排列
        ...
        fp[0][t - 1] = Nt - c[0][t - 1]  # c[0][t - 1]为TP，NT=TP+FP
        m[0][t - 1] = g[0][t - 1] - c[0][t - 1]  # m即是FN，g即是TP+FN
