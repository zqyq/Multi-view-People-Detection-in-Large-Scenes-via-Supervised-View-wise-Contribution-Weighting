...
def prediction_vis(pred, gt, save_dir):
    fig = plt.figure()
    subplt0 = fig.add_subplot(121, title="gt")
    subplt1 = fig.add_subplot(122, title="pred")
    subplt0.imshow(gt.squeeze().cpu().detach().numpy().squeeze())
    subplt1.imshow(pred.cpu().detach().numpy().squeeze())
    plt.savefig(save_dir)
    plt.close(fig)


def vertical_prediction_vis(pred, gt, save_dir):
    fig = plt.figure()
    subplt0 = fig.add_subplot(211, title="gt")
    subplt1 = fig.add_subplot(212, title="pred")
    subplt0.imshow(gt.squeeze().cpu().detach().numpy().squeeze())
    subplt1.imshow(pred.cpu().detach().numpy().squeeze())
    plt.savefig(save_dir)
    plt.close(fig)
